...
class App(eng.Engine):
	...
	def setupSession(self):
		...
		mesh = np.load("res/plane.npz")
		
		self.vertex_data = mesh['VBO']
		self.vertex_index = mesh['IBO']
		...
		# self.vertex_index = np.array([
			# [0, 1, 2], # First triangle
			# [2, 3, 0]  # Second triangle
			# ], dtype=np.uint32)
		
		self.vbo = GL.glGenBuffers(1) # vertex data
		GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self.vbo) # Select self.vbo
		# Assign data to selected buffer:
		GL.glBufferData(GL.GL_ARRAY_BUFFER, self.vertex_data, GL.GL_STATIC_DRAW)
		
		
		self.ibo = GL.glGenBuffers(1) # vertex index
		GL.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER, self.ibo) # Select self.ibo
		# Assign data to selected buffer:
		GL.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER, self.vertex_index, GL.GL_STATIC_DRAW)
